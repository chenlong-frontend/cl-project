...
tool.ajax=function(opt,backFn){
    ...
    $.ajax({
        ...
        success: function(data) {
            console.log(data);
            if(data.code === 0) {
                if ($.isFunction(backFn)){backFn(data)}

            } else {
                mui.alert(data["msg"],"提示","确认",function () {
                    console.log("错误")
                });
            }
        },
...
tool.cookie = (function () {
    ...
    function setCookie(name,value,time) {
        ...
        document.cookie=name+ "=" +encodeURI(value)+ ((time===null) ? "" : ";expires="+exp.toGMTString())
    }
...
tool.emptyCheck = function (opt) {
    var flag = true;
    function check(obj) {
        var lenCheck = obj.data.length === 0;
