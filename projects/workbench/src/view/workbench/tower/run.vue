  ...
  <div class="runBox">
    <el-container>
                ...
                <el-row>
                  <el-col :span="8">
                    <div class="grid-content bg-purple">
                      <div class="boxItem">
                        <p>当前运行中塔吊数量</p>
                        <span class="count">{{nowRun}}</span> <span class="unit">台</span>
                      </div>
                    </div>
                  </el-col>
                  <el-col :span="8">
                    <div class="grid-content bg-purple-light">
                      <div class="boxItem">
                        <p>累计运行的塔吊总数量</p>
                        <span class="count">{{tenDayRun}}</span> <span class="unit">台</span>
                      </div>
                    </div>
                  </el-col>
                  <el-col :span="8">
                    <div class="grid-content bg-purple">
                      <div class="boxItem">
                        <p>当日运行的塔吊总次数</p>
                        <span class="count">{{todayRun}}</span> <span class="unit">台</span>
                      </div>
                    </div>
                  </el-col>
                </el-row>
      ...
      <el-aside width="40%" class="rightBoxs">
        <ibox title="塔吊实时运行列表" class="policeList">
          ...
          <el-dialog
            title="运行详情"
            :visible.sync="showRunDetail"
            width="80%"
            :append-to-body="true">
            <run-detail :runInfoID="runInfoID"></run-detail>
          </el-dialog>
          <!-- 报警详情对话框结束 -->
          ...
          <el-dialog
            ...
            width="80%"
            :append-to-body="true">
            <run-list :show='this.showPolideList' @toParent='showChild'></run-list>
          </el-dialog>
          <!-- 报警列表对话框结束 -->
        </ibox>
      </el-aside>
...
import runList from '@/view/workbench/tower/run/runList.vue'
import runDetail from '@/view/workbench/tower/run/runDetail.vue'
import request from '@/api/workbench/tower/run.js'
...
export default {
  name: 'run',
  components: {ibox, runList, runDetail},
  data () {
    return {
  ...
  methods: {
      ...
      request.getRunNums().then(
        data => {
          if (data.data.code === 0) {
            ...
            this.tenDayRun = data.data.data.tenDayRun
    ...
    getRunList () {
      request.getRunList().then(
        data => {
          if (data.data.code === 0) {
            this.warnInfos = data.data.data.runInfos
          } else this.$message.error('获取失败')
        },
        error => {
          console.log(error)
    ...
    getRunChart () {
      request.getRunChart(this.flag).then(
        data => {
          if (data.data.code === 0) {
